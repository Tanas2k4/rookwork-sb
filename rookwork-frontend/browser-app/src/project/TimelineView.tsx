...
import { MOCK_TASKS as RAW_TASKS } from "../mocks/board";
...
import type { GanttTask, ViewMode } from "./timeline/timelineUtils";
import {
  taskToGantt,
...
const TASKS: GanttTask[] = RAW_TASKS.map(taskToGantt);
...
export default function TimelineView() {
  ...
  // Timeline bounds
  // Include "today" so the red line is always in-range
  const allDates = TASKS.flatMap((t) => [t.start, t.end]).concat(today);
  ...
  const groups = Array.from(new Set(TASKS.map((t) => t.group || "Other"))).sort(
  ...
  return (
    <div className="flex flex-col h-full bg-white text-gray-700 select-none overflow-hidden">
            ...
            >
              {columns.map((col, i) => (
                <div
                  key={i}
                  style={{
                    width: col.days * colWidth,
                    minWidth: col.days * colWidth,
                  }}
                    ...
                    {!collapsedGroups.has(group) &&
                      groupTasks.map((task) => (
                        <div
                          key={task.id}
                          style={{ height: ROW_HEIGHT }}
                          className={`border-b border-gray-100 transition-colors ${selectedTask === task.id ? "bg-indigo-50/60" : ""}`}
                        />
                      ))}
                  </div>
                );
              ...
              {todayX > 0 && todayX < totalWidth && (
                <div
                  ...
                  style={{
                    ...
                    height: "100%",
                    background:
                      "linear-gradient(to bottom, #f43f5e, transparent)",
                    zIndex: 10,
                  }}
                >
                  <div className="absolute -top-1 left-1/2 -trangray-x-1/2 w-2 h-2 rounded-full bg-rose-500" />
                </div>
              )}
          ...
          <div className="flex items-center gap-6 flex-shrink-0">
                ...
                <div className="w-24 h-1.5 rounded-full bg-gray-200 overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: selectedTaskData.color,
                    }}
                  />
                </div>
            ...
            <div className="flex flex-col items-center gap-1">
              ...
              <div className="flex items-center gap-1.5">
                <span
                  className={`w-1.5 h-1.5 rounded-full ${STATUS_CONFIG[selectedTaskData.status || "todo"].dot}`}
                />
      ...
      <style>{`
        ...
        @keyframes slide-up { from { opacity:0; transform:trangrayY(8px); } to { opacity:1; transform:trangrayY(0); } }
