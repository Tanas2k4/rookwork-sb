...
import { MILESTONES } from "../../mocks/overview";
...
export default function Milestones({ animated }: { animated: boolean }) {
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm col-span-3">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-gray-400">
          {MILESTONES.filter((m) => m.status === "done").length}/{MILESTONES.length} completed
        </span>
      </div>
      <div className="grid grid-cols-5 gap-3">
        {MILESTONES.map((m) => {
          const barCls =
            m.status === "done" ? "bg-green-600"
            : m.status === "in_progress" ? "bg-blue-600"
            : "bg-gray-400";
          return (
            <div
              key={m.id}
              className="bg-gray-50 border border-gray-200 rounded-xl p-4 hover:bg-white hover:shadow-md hover:-translate-y-0.5 transition-all"
            >
